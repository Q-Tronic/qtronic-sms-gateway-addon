...
void Sim800LComponent::update() {
  ...
  if (this->last_activity_ms_ == 0) {
    this->last_activity_ms_ = now;
  }

  // The bundled component can remain stuck forever after the modem loses
  // power while an acknowledgement is pending. Recover the state machine
  // without injecting commands from a second UART consumer or rebooting ESP.
  if (static_cast<uint32_t>(now - this->last_activity_ms_) >= 15000) {
    ESP_LOGW(TAG, "Modem command timed out; resetting SIM800 state machine");
...
void Sim800LComponent::parse_cmd_(std::string message) {
  ...
  this->last_activity_ms_ = millis();
  this->last_response_ms_ = this->last_activity_ms_;
...
void Sim800LComponent::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  Q-Tronic timeout recovery patch: enabled");
